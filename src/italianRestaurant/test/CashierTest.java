...
public class CashierTest extends TestCase
{
	...
	public void setUp() throws Exception{
		super.setUp();		
		cashier = new ItalianCashierRole("americanRestaurantCashier", restaurant);		
	...
	public void testOneNormalCustomerScenario()
	{
		...
		//check preconditions
		assertEquals("AmericanRestaurantCashier should have 0 bills in it. It doesn't.",cashier.BillOrders.size(), 0); 
		...
		assertEquals("AmericanRestaurantCashier should have 1 bill in it. It doesn't.", cashier.BillOrders.size(), 1);
		
		assertFalse("AmericanRestaurantCashier's scheduler should have returned false (no actions to do on a billorder from a waiter), but didn't.", cashier.pickAndExecuteAnAction());
		...
		assertTrue("AmericanRestaurantCashier's scheduler should have returned true (needs to react to customer's HereIsMoney), but didn't.", 
					cashier.pickAndExecuteAnAction());
		
		//check postconditions for step 3 / preconditions for step 4
		assertTrue("AmericanRestaurantMockCustomer should have logged an event for receiving \"HereIsYourTotal\" with the correct balance, but his last event logged reads instead: " 
				+ customer.log.getLastLoggedEvent().toString(), customer.log.containsString("Received HereIsYourTotal from americanRestaurantCashier. Total = 7.98"));
		...
		assertTrue("AmericanRestaurantMockCustomer should have logged an event for receiving \"HereIsYourChange\" with the correct change, but his last event logged reads instead: " 
				+ customer.log.getLastLoggedEvent().toString(), customer.log.containsString("Received HereIsYourChange from americanRestaurantCashier. Change = 0.0"));
	...
	public void testOneNormalMarketScenario()
	{
		...
		//check preconditions
		assertEquals("AmericanRestaurantCashier should have 0 bill in it. It doesn't.", cashier.BillFoods.size(), 0);
		
		cashier.msgRestockingBill(market1, "Steak", 271.83);
		//step 1
		assertEquals("AmericanRestaurantCashier should have 1 bill in it. It doesn't.", cashier.BillFoods.size(), 1);
	...
	public void testTwoMarketsScenario()
	{
		...
		//step 1
				assertEquals("AmericanRestaurantCashier should have 2 bill in it. It doesn't.", cashier.BillFoods.size(), 2);
