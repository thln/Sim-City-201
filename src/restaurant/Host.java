...
public class Host extends Agent {
	...
	private void greetCustomer() {
		...
		if (fullTableIterator == NTABLES) {
			informCustomerRestaurantFull(newCustomers.get(0).restaurantCustomer);
		}
		else {
	...
	private void assignCustomer(myCustomer MC, Table table, Waiter waiter) {
		print("Assigning customer " + MC.restaurantCustomer.getCustomerName() + " to waiter");
		addCustomerToWaiter(waiter);
		waiter.msgPleaseSeatCustomer(table.tableNumber, MC.restaurantCustomer, MC.xHome, MC.yHome);
		table.setOccupant(MC.restaurantCustomer);
	...
	private void informCustomerRestaurantFull(RestaurantCustomer restaurantCustomer) {
		restaurantCustomer.msgTablesAreFull();
		for(myCustomer MC : newCustomers) {
			if (MC.restaurantCustomer.equals(restaurantCustomer)) {
	...
	private class myCustomer {
		...
		myCustomer(RestaurantCustomer restaurantCustomer, int xHome, int yHome) {
