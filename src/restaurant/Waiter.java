...
public class Waiter extends Agent implements Waiter{
	 ...
	 * Messages
	 */
	public void msgPleaseSeatCustomer(int tableNumber, RestaurantCustomer restaurantCustomer, int xHome, int yHome) {
		myCustomers.add(new myCustomer(restaurantCustomer, tableNumber, xHome, yHome));
		print(restaurantCustomer.getCustomerName() + " was added to myCustomers list");
		stateChanged();
	}
	...
	public void msgReadyToOrder(RestaurantCustomer restaurantCustomer) {
		for (myCustomer myCust : myCustomers) {
			if (myCust.restaurantCustomer == restaurantCustomer) {
	...
	public void msgHeresMyOrder(RestaurantCustomer restaurantCustomer, String choice) {
		for (myCustomer myCust : myCustomers) {
			if (myCust.restaurantCustomer == restaurantCustomer) {
	...
	public void msgOrderIsReady(int table, String choice) {
		...
		for (myCustomer MC: myCustomers) {
			if (MC.tableNumber == table)
				order = new Order(table, choice, MC.restaurantCustomer);
		}
	...
	public void msgIWantMyCheck(RestaurantCustomer cust) {
		for (myCustomer myCust : myCustomers) {
			if (myCust.restaurantCustomer == cust) {
				myCust.setWantCheck();
				stateChanged();
			}
		}
	...
	public void msgHereIsCheck(int tableNumber, double checkAmount) {
		for (myCustomer myCust : myCustomers) {
			if (myCust.tableNumber == tableNumber) {
				...
				print("Recieved " + myCust.restaurantCustomer.getCustomerName() + "'s check of $" + myCust.CheckAmount);
				stateChanged();
			}
		}
	}
	...
	private void seatCustomer(myCustomer MC) {
		...
		MC.restaurantCustomer.msgPleaseFollowMe(MC.tableNumber, menu, this);
		isInLobby = false;
		DoSeatCustomer(MC.restaurantCustomer, MC.tableNumber);
		try {
			atDestination.acquire();
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
		...
		print("Finished seating " + MC.restaurantCustomer.getCustomerName());
		MC.setSeated();
	}

	// The animation DoXYZ() routines
	private void DoSeatCustomer(RestaurantCustomer restaurantCustomer, int table) {
		//Notice how we print "customer" directly. It's toString method will do it.
		//Same with "table"
		isInLobby = false;
		print("Seating " + restaurantCustomer + " at " + table);
		waiterGui.DoBringToTable(restaurantCustomer, table);
	}

	private void takeOrder(myCustomer MC) {
		isInLobby = false;
		print(MC.restaurantCustomer.getCustomerName() + " wants to order");

		for (myCustomer myCust : myCustomers) {
			if (myCust.restaurantCustomer == MC.restaurantCustomer) {
		...
		MC.restaurantCustomer.msgWhatWouldYouLike();
	}

	private void placeOrder(myCustomer MC) {
		isInLobby = false;
		print("Placing " + MC.restaurantCustomer.getCustomerName() + "'s order");
	...
	private void retakeOrder(myCustomer MC) {
		...
		print("Asking " + MC.restaurantCustomer.getCustomerName() + " for a re-order");
		MC.restaurantCustomer.msgPleaseReorder(new Menu().remove(MC.choice));
	...
	private void deliverOrder() {
		...
		waiterGui.DoLeaveCustomer();
		readyOrders.get(0).restaurantCustomer.msgHeresYourOrder(readyOrders.get(0).choice);
		cashier.msgComputeBill(readyOrders.get(0).choice, readyOrders.get(0).tableNumber, this);

		//Changing customer state to "Got Food"
		for (myCustomer MC : myCustomers) {
			if (MC.restaurantCustomer.equals(readyOrders.get(0).restaurantCustomer))
				MC.setGotFood();	
		}
	...
	private void giveCheck(myCustomer MC) {
		...
		MC.restaurantCustomer.msgHeresYourCheck(MC.CheckAmount);
		MC.state = customerState.GaveCheck;
	}

	private void clearTable(myCustomer MC){
		print(MC.restaurantCustomer.getCustomerName() + " is leaving " + MC.tableNumber);
		host.msgLeavingTable(MC.restaurantCustomer, this);
