...
public class Cashier extends Agent implements Cashier {
	...
	public void msgPayment(String choice, double amount, RestaurantCustomer restaurantCustomer) {
		synchronized(Payments){
			print("Received payment from " + restaurantCustomer.getCustomerName());
			log.add(new LoggedEvent("Received payment from " + restaurantCustomer.getCustomerName()));
			Payments.add(new Payment(choice, amount, restaurantCustomer));
			stateChanged();
		}
	}
	...
	private void GiveChange() {
		if (Payments.get(0).payment < foodPrices.get(Payments.get(0).choice)) {
			Payments.get(0).restaurantCustomer.msgGoToJail();
			Payments.remove(0);
			return;
		}
		...
		Payments.get(0).restaurantCustomer.msgHeresYourChange(change);
		print("Gave change to customer " + Payments.get(0).restaurantCustomer.getName());
		Payments.remove(0);
	}
	...
	public class Payment {
		...
		public RestaurantCustomer restaurantCustomer;

		Payment(String choice, double payment, RestaurantCustomer restaurantCustomer) {
			this.choice = choice;
